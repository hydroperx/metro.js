export function computePosition(referenceElement: HTMLElement, positioningElement: HTMLElement, options: ComputePositionOptions | null = null): [number, number]
{
    ...
    switch (prefer)
    {
        case "bottom":
        {
            if (referenceRect.bottom + margin + positioningRect.height < viewportHeight)
            {
                resolution = "bottom";
            }
...
export function fitViewportPosition(positioningElement: HTMLElement, [x, y]: [number, number]): [number, number]
{
    const positioningRect = positioningElement.getBoundingClientRect();
    ...
    y = y + positioningRect.height < viewportHeight ? y : y - positioningRect.height;
    y = y < 0 ? 0 : y;

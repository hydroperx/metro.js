...
const Div = styled.div<{
  ...
  display: ${($) =>
    ($.$visible ?? true) ? ($.$inline ? "inline-flex" : "flex") : "none"};
  flex-direction: row;
...
export function HGroup(options: HGroupOptions) {
  ...
  if (options.easeOutPosition) {
    transition =
      "left 200ms ease-out, top 200ms ease-out, right 200ms ease-out, bottom 200ms ease-out";
  }
  ...
  if (options.easeInPosition) {
    transition =
      (transition ? transition + ", " : "") +
      "left 200ms ease-in, top 200ms ease-in, right 200ms ease-in, bottom 200ms ease-in";
  }
  if (options.easeInTransform) {
    transition =
      (transition ? transition + ", " : "") + "transform 200ms ease-in";
  }
  ...
  if (options.easeInOpacity) {
    transition =
      (transition ? transition + ", " : "") + "opacity 200ms ease-in";
  }
  ...
  const handle_wheel = (e: WheelEvent): void => {
    ...
    if (options.wheelHorizontal && e.deltaMode == 0) {
      ...
      let multiplier = 2;
      if (
        last_wheel_timestamp != -1 &&
        ((last_wheel_timestamp > Date.now() - 600 &&
          last_wheel_timestamp < Date.now() - 20) ||
          (last_fast_wheel_timestamp !== -1 &&
            last_fast_wheel_timestamp > Date.now() - 100))
      )
  ...
  return (
    <Div
      ref={(node) => {
        ref.current = node;
        if (typeof options.ref == "function") options.ref(node);
        else if (options.ref) options.ref.current = node;
      }}

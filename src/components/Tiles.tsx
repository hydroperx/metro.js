...
export function Tiles(options: TilesOptions) {
  assert(
    ...
    "Vertical tiles not supported currently.",
  );
  ...
  function init_tiles1(): void {
    tiles1 = new Tiles1({
      ...
      maxHeight: 6,
      tileTransition:
        "opacity 0.2s, transform 0.2s ease-out, scale 0.2s ease-out",
    });
      ...
      button.addEventListener("click", (e) => {
        ...
        if (contextTimestamp === -1 || contextTimestamp < Date.now() - 100) {
          ...
          if (button.getAttribute("data-selection-mode") === "true")
            ...
            options.tileClick?.(
              (e.currentTarget as HTMLButtonElement).getAttribute("data-id")!,
            );
  ...
  function tile_simulated_context_menu(tile_button: HTMLButtonElement): void {
    if (tile_button.getAttribute("data-checked") == "true") {
      ...
      if (all_buttons.every(([, y]) => !y)) mode_signal({ selection: false });
      options.checkedChange?.(
        all_buttons.filter(([, y]) => y).map(([id]) => id as string),
      );
      return;
    }
  ...
  function on_key_down(e: KeyboardEvent): void {
    ...
    if (e.key.toLowerCase() == "a" && e.ctrlKey && !e.shiftKey && !e.altKey) {
      ...
      if (buttons.length != 0) mode_signal({ selection: true });
      options.checkedChange?.(
  ...
  function set_tile_pages(
    ...
    // Setup animation
    let anim_prefix_1 =
      page_elements.length <= 1 ? "" : "Tiles--slideY" + page_elements.length;
